function setup_my_rbenv {
  if [ -d ~/.rbenv ] && command -v rbenv > /dev/null; then
    ...
    zinit ice lucid wait"0c" as"null" atclone"setup_my_rbenv_default_gems"
    zinit light rbenv/rbenv-default-gems

    zinit ice lucid wait"0c" as"null" atclone"setup_my_rbenv_each"
    zinit light rbenv/rbenv-each

    zinit ice lucid wait"0c" as"null" atclone"setup_my_ruby_build"
